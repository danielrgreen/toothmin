...
def compare(model_isomap, model_isomap_2, model_isomap_3, data_isomap, w_iso_hist, prior_rate, score_max=100., data_sigma=0.20, sigma_floor=0.05):
    ...
    model_sigma = np.std(model_isomap, axis=2)# The factor to fullres is 7.18x + .0032
    ...
    prior_score_rate = prior_rate_change(w_iso_hist, prior_rate) # rate prior
...
def water_hist_likelihood(w_iso_hist, PO4_t, PO4_pause, PO4_flux, **kwargs):
    ...
    d_O2 = kwargs.get('d_O2', 21.5)
    d_feed = kwargs.get('d_feed', 23.3)
    ...
    M2_inverse_blood_hist = blood_delta(21.5, w_iso_hist, 23.3, **metabolic_kw)
...
def dicynodont_to_sheep(conversion_times, x1, x2, x3, x4, max1, a2, s2, o2, max2):

    t1_ext = x1*conversion_times**3 - x2*conversion_times**2 + x3*conversion_times - x4
    t1_pct = t1_ext / max1
    t2_ext = t1_pct * max2
    converted_times = (spec.erfinv((a2+t2_ext-max2)/a2) + (o2*s2)) / s2
...
def sheep_to_dicynodont(conversion_times, a1, s1, o1, max1, x1, x2, x3, x4, max2):
    ...
    for e2 in t2_ext:
        ...
        idx = np.isreal(t)
        if np.any(idx):
            converted_times.append(float(t[idx][0]))
        else:
            converted_times.append(np.nan)

    print 'Converted times: {}'.format(converted_times)
...
def fit_tooth_data(data_fname, model_fname='equalsize_jul2015a.h5', **kwargs):
    ...
    data_dicynodont = np.array([7.75, 7.75, 9.9, 8.2, 7.8, 7.5, 7.6, 7.9, 7.4, 7.75, 6.7, 6.9, 7.8, 9.25, 7.6])
    data_isomap = data_dicynodont
    isomap_shape = (len(data_isomap), 1)
    isomap_data_x_ct = np.ones(isomap_shape[0])
    print data_isomap, isomap_shape, isomap_data_x_ct

    print 'loading tooth model ...'
    tooth_model_lg = ToothModel(model_fname)
    tooth_model = tooth_model_lg.downsample_model((isomap_shape[0]+5, isomap_shape[1]+5), 1) # Model resampling size, sample number
    ...
    data_guess = guess_first(data_isomap, 21.5, 23.3, 27)
    ...
    M2_inverse_days = np.arange(84., 84.+np.size(M2_inverse_water_hist))
    M2_inverse_blood_hist = blood_delta(21.5, M2_inverse_water_hist, 23.3, **forward_metabolic_kw)
    ...
    guess_w_hist = spline_input_signal(guess_g, 14, 1)
    guess_score = compare(inverse_model_PO4, inverse_model_PO4_2, inverse_model_PO4_2, data_isomap, guess_w_hist, prior_rate)
    g_rate_a, g_rate_b, g_rate_c, g_rate_d = prior_rate_change(guess_w_hist, 0.25), prior_rate_change(guess_w_hist, 0.33), prior_rate_change(guess_w_hist, 0.66), prior_rate_change(guess_w_hist, 1.00)
    w_rate_a, w_rate_b, w_rate_c, w_rate_d = prior_rate_change(M2_inverse_water_hist, 0.25), prior_rate_change(M2_inverse_water_hist, 0.33), prior_rate_change(M2_inverse_water_hist, 0.66), prior_rate_change(M2_inverse_water_hist, 1.00)
    ...
    m2_dicynodont_params = np.array([67.974, 0.003352, -25.414, 41., .00000019415, .000091976, .0270315, .0502, 4.89])
    dicynodont_days = sheep_to_dicynodont(days_spl_962, *m2_dicynodont_params)
    dicynodont_days = dicynodont_days - dicynodont_days[0]
    water_spl_tmp = np.ones(dicynodont_days.size)
    blood_spl_tmp = np.ones(dicynodont_days.size)
    PO4_spl_tmp = np.ones(dicynodont_days.size)
    for k,d in enumerate(dicynodont_days):
        d = int(d)
        water_spl_tmp[d:] = M2_inverse_water_hist[k]
        blood_spl_tmp[d:] = M2_inverse_blood_hist[k]
        PO4_spl_tmp[d:] = M2_inverse_PO4_eq[k]
    M2_inverse_water_hist = water_spl_tmp
    M2_inverse_blood_hist = blood_spl_tmp
    M2_inverse_PO4_eq = PO4_spl_tmp

    data_isomap.shape = isomap_shape

    fig = plt.figure()
    ax1 = fig.add_subplot(4,1,1)
    days = np.arange(len(M2_inverse_water_hist))
    ...
    for s in list_water_results[:-1]:
        ...
        s_tmp = np.ones(dicynodont_days.size)
        for k,d in enumerate(dicynodont_days):
            d = int(d)
            s_tmp[d:] = s[k]
        s = s_tmp
    ...
    cimg2 = ax2.imshow(data_isomap.T, aspect='equal', interpolation='nearest', origin='lower', cmap='bwr', vmin=6.5, vmax=9.5)
    cax2 = fig.colorbar(cimg2)

    ax3 = fig.add_subplot(5,1,3)
    ax3text = 'Inverse model result PO4 {0}'.format(number)
    ax3.text(3, 5, ax3text, fontsize=8)
    cimg3 = ax3.imshow(np.mean(inverse_model_PO4, axis=2).T, aspect='equal', interpolation='nearest', origin='lower', cmap='bwr', vmin=6.5, vmax=9.5)
    ...
    cimg4 = ax4.imshow(residuals, aspect='equal', interpolation='nearest', origin='lower', cmap='RdGy') # Residuals
    ...
    cimg5 = ax5.imshow(sigma.T, aspect='equal', interpolation='nearest', origin='lower', cmap='RdGy') # Residuals
    cax5 = fig.colorbar(cimg5)

    fig.savefig('dtest_r{0}_18p6_{1}_{2}a.svg'.format(prior_rate, number, t_save), dpi=300, bbox_inches='tight')
    ...
    fig.savefig('dtest_r{0}_18p6_{1}_{2}b.svg'.format(prior_rate, number, t_save), dpi=300, bbox_inches='tight')
    ...
    ax1text2 = 'guess score = {:.2f}'.format(guess_score)
    ax1.text(100, -1, ax1text2, fontsize=8)
    ax1text3 = 'guess rate scores for 0.25, 0.33, 0.66 and 1.00 = {:.1f},{:.1f},{:.1f},{:.1f}'.format(g_rate_a, g_rate_b, g_rate_c, g_rate_d)
    ax1.text(100, -2, ax1text3, fontsize=8)
    ax1text3 = 'opt rate scores for 0.25, 0.33, 0.66 and 1.00 = {:.1f},{:.1f},{:.1f},{:.1f}'.format(w_rate_a, w_rate_b, w_rate_c, w_rate_d)
    ax1.text(100, -3, ax1text3, fontsize=8)
    ax1.set_ylim(min(M2_inverse_water_hist)-6., max(M2_inverse_water_hist)+6.)
    ax1.set_xlim(0, 300)
    fig.savefig('dtest_r{0}_18p6_{1}_{2}b2.svg'.format(prior_rate, number, t_save), dpi=300, bbox_inches='tight')
    ...
    for s in list_water_results[:-1]:
        ...
        ax1.plot(days, s, 'b-', alpha=0.03)
    ax1.text(350, -18, textstr, fontsize=8)
    ax1.set_ylim(min(M2_inverse_water_hist)-2., max(M2_inverse_water_hist)+4.)
    ax1.set_xlim(0, 300)

    fig.savefig('dtest_r{0}_18p6_{1}_{2}b3.svg'.format(prior_rate, number, t_save), dpi=300, bbox_inches='tight')

    fig = plt.figure()
    plt.hist(hist_list, bins=np.logspace(0.0, 5.0, 30), alpha=.6)
    plt.gca().set_xscale("log")
    fig.savefig('dtest_r{0}_18p6_{1}_{2}c.svg'.format(prior_rate, number, t_save), dpi=300, bbox_inches='tight')
    ...
    ax1.set_xlim(0, 300)
    fig.savefig('dtest_r{0}_18p6_{1}_{2}e3.svg'.format(prior_rate, number, t_save), dpi=300, bbox_inches='tight')
    ...
    fig.savefig('dtest_r{0}_18p6_{1}_{2}f.svg'.format(prior_rate, number, t_save), dpi=300, bbox_inches='tight')
    ...
    #ax1.plot(score_prior_counter_array[:,0], score_prior_counter_array[:,1], 'k-', label='raw score') # score
    ax1.legend(fontsize=8)
    fig.savefig('dtest_r{0}_18p6_{1}_{2}g.svg'.format(prior_rate, number, t_save), dpi=300, bbox_inches='tight')
    ...
    for s in list_water_results[:-1]:
        ...
        ax1.plot(days, s, 'b-', alpha=0.03)
    ax1.text(350, -6, textstr, fontsize=8)
    ax1.set_ylim(np.min(M2_inverse_water_hist)-2., np.max(M2_inverse_water_hist)+2.)
    ax1.set_xlim(0, 300)
    #ax2 = ax1.twiny()
    #ax2.plot(np.linspace(0, 42, len(normal_samples)), normal_samples, 'ko')
    #ax2.set_xlim(0, 42)
    #ax2.set_ylim(min(water_hist)-6., max(water_hist)+3.)
    fig.savefig('dtest_r{0}_18p6_{1}_{2}h.svg'.format(prior_rate, number, t_save), dpi=300, bbox_inches='tight')
    ...
    fig.savefig('dtest_r{0}_18p6_{1}_{2}i.svg'.format(prior_rate, number, t_save), dpi=300, bbox_inches='tight')

    np.savetxt('dtest_r{0}_18p6_{1}_{2}.csv'.format(prior_rate, number, t_save), score_prior_counter_array, fmt='%.4f', delimiter=',')
