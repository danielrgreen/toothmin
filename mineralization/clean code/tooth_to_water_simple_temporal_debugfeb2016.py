...
def gen_isomaps(iso_shape, iso_data_x_ct, tooth_model, blood_step, day=-1):
    ...
    print 'model isomap shape', model_isomap.shape
    print 'calcing sigma'
    sigma = np.std(model_isomap, axis=0)
    print 'calcing mean'
    mean = np.mean(model_isomap, axis=0)
    m_mean = np.ma.masked_array(mean, np.isnan(mean))
    m_sigma = np.ma.masked_array(sigma, np.isnan(sigma))
    mean_sigma = np.mean(m_sigma[:,:,-1])
    percentile_sigma = np.percentile(m_sigma[:,:,-1], (5., 50., 95.))
    mean_mean = np.mean(m_mean[:,:,-1])
    percentile_mean = np.percentile(m_mean[:,:,-1], (5., 50., 95.))

    fig = plt.figure()
    ax1 = fig.add_subplot(2,1,1)
    ax1text = 'sigma, {0}, {1}'.format(mean_sigma, percentile_sigma)
    ax1.text(1, 1, ax1text, fontsize=8)
    cimg1 = ax1.imshow(sigma[:,:,-1].T, aspect='auto', interpolation='nearest', origin='lower', cmap='bwr')
    cax1 = fig.colorbar(cimg1)

    ax2 = fig.add_subplot(2,1,2)
    ax2text = 'mean, {0}, {1}'.format(mean_mean, percentile_mean)
    ax2.text(1, 1, ax2text, fontsize=8)
    cimg2 = ax2.imshow(mean[:,:,-1].T, aspect='auto', interpolation='nearest', origin='lower', cmap='bwr')
    cax2 = fig.colorbar(cimg2)
    fig.savefig('m2exp_fullres_mean_sigma_2016_02_08.svg')
...
def compare(model_isomap, data_isomap, w_iso_hist, score_max=100., data_sigma=0.25, sigma_floor=0.50):
    ...
    sigma1 = np.std(model_isomap, axis=2)
    sigma2 = np.sqrt(sigma1**2. + data_sigma**2. + sigma_floor**2.)

    fig = plt.figure()
    ax1 = fig.add_subplot(2,1,1)
    ax1text = 'sigma1'
    ax1.text(21, 3, ax1text, fontsize=8)
    cimg1 = ax1.imshow(sigma1.T, aspect='auto', interpolation='nearest', origin='lower', cmap='RdGy') # Residuals
    cax1 = fig.colorbar(cimg1)
    ax2 = fig.add_subplot(2,1,2)
    ax2text = 'sigma2'
    ax2.text(21, 3, ax1text, fontsize=8)
    cimg2 = ax2.imshow(sigma2.T, aspect='auto', interpolation='nearest', origin='lower', cmap='RdGy') # Residuals
    cax2 = fig.colorbar(cimg2)
...
def water_hist_prob(w_params, **kwargs):
    ...
    month_1exp = np.array([-18.50, -17.93+20.00, -12.16, -12.08, -6.88, -7.00, -7.49, -5.60, -8.87, -13.91, -14.20, -23.70]) # North Platte, Nebraska
    month_2exp = np.array([-18.50, -17.93, -12.16, -12.08+20.00, -6.88, -7.00, -7.49, -5.60, -8.87, -13.91, -14.20, -23.70]) # North Platte, Nebraska
    month_3exp = np.array([-18.50, -17.93, -12.16, -12.08, -6.88, -7.00+20.00, -7.49, -5.60, -8.87, -13.91, -14.20, -23.70]) # North Platte, Nebraska
    ...
    m_1exp = spline_input_signal(np.concatenate((month_1exp, month_1exp)), 30, 1)
    m_2exp = spline_input_signal(np.concatenate((month_2exp, month_2exp)), 30, 1)
    m_3exp = spline_input_signal(np.concatenate((month_3exp, month_3exp)), 30, 1)
