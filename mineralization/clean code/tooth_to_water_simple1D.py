...
def compare(model_isomap, model_isomap_2, model_isomap_3, data_isomap, w_iso_hist, score_max=100., data_sigma=0.20, sigma_floor=0.05):
    '''
    ...
    time_box = np.concatenate((model_isomap_1D, model_isomap_1D2, model_isomap_1D3), axis=1)
    time_sigma = np.std(time_box, axis=1)

    mu = np.median(model_isomap_1D, axis=1)
    data_isomap = np.mean(m_data, axis=1)

    model_sigma = np.std(model_isomap_1D, axis=1) * 7. # In 2D sampling we also multiply by 7 to reflect MCMC results.
    sigma = np.sqrt(model_sigma**2. + data_sigma**2. + time_sigma**2 + sigma_floor**2.)
...
def water_hist_likelihood(w_iso_hist, switch_params, PO4_t, PO4_pause, PO4_flux, **kwargs):
    ...
    score = compare(inverse_model_PO4, inverse_model_PO4_2, inverse_model_PO4_3, data_isomap, w_iso_hist)
...
def fit_tooth_data(data_fname, model_fname='equalsize_jul2015a.h5', **kwargs):
    ...
    mu_1D = np.median(model_isomap_1D, axis=1)
    ...
    sigma = np.sqrt(0.20**2. + time_sigma**2 + 0.05**2.)
    ...
    cimg2 = ax2.imshow(data_isomap_1D.T, aspect='equal', interpolation='nearest', origin='lower', cmap='bwr')
    cax2 = fig.colorbar(cimg2)

    ax3 = fig.add_subplot(4,1,3)
    ax3text = 'Inverse model result - PO4'
    ax3.text(15, 1, ax3text, fontsize=8)
    cimg3 = ax3.imshow(mu_1D.T, aspect='equal', interpolation='nearest', origin='lower', cmap='bwr')
    ...
    fig.savefig('1D_{0}_rate1o3_18p6_sigma10_{1}a.svg'.format(number, t_save), dpi=300, bbox_inches='tight')
    ...
    fig.savefig('1D_{0}_rate1o3_18p6_sigma10_{1}b.svg'.format(number, t_save), dpi=300, bbox_inches='tight')
    ...
    ax1.set_xlim(84, 550)
    fig.savefig('1D_rate1o3_18p6_sigma10_{0}_{1}b2.svg'.format(number, t_save), dpi=300, bbox_inches='tight')
    ...
    fig.savefig('1D_rate1o3_18p6_sigma10_{0}_{1}b3.svg'.format(number, t_save), dpi=300, bbox_inches='tight')

    fig = plt.figure()
    plt.hist(hist_list, bins=np.logspace(0.0, 5.0, 30), alpha=.6)
    plt.gca().set_xscale("log")
    plt.savefig('1D_{0}_rate1o3_18p6_sigma10_{1}c.svg'.format(number, t_save), dpi=300, bbox_inches='tight')

    fig = plt.figure()
    ax1 = fig.add_subplot(1,1,1)
    ax1.hist(for_inv_diff, bins=np.linspace(-10.0, 10.0, 20), alpha=.6)
    fig.savefig('1D_rate1o3_18p6_sigma10_{0}_{1}d.svg'.format(number, t_save), dpi=300, bbox_inches='tight')
    ...
    ax1.set_ylim(0, 220)
    fig.savefig('1D_rate1o3_18p6_sigma10_{0}_{1}e.svg'.format(number, t_save), dpi=300, bbox_inches='tight')
    ...
    fig.savefig('1D_rate1o3_18p6_{0}_{1}f.svg'.format(number, t_save), dpi=300, bbox_inches='tight')

    np.savetxt('1D_rate1o3_18p6_{0}_{1}.csv'.format(number, t_save), score_prior_counter_array, fmt='%.4f', delimiter=',')
