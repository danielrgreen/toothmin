...
def fit_tooth_data(data_fname, model_fname='equalsize_jul2015a.h5', **kwargs):
    ...
    switch_high, switch_low, switch_guess = [0., -19.4, 74., 62.], [0., -19.4, 74., 62.], [0., -19.4, 74., 62.]
    for j,k in enumerate(switch_high):
        max_g.append(switch_high[j])
        min_g.append(switch_low[j])
        guess_g.append(switch_guess[j])
    ...
    forward_score = compare(forward_model_M1_PO4_hist, data_isomap, forward_962_water_hist, forward_M2_switch_days_for_check, prior_rate)
    ...
    cimg2 = ax2.imshow(data_isomap.T, aspect='equal', interpolation='nearest', origin='lower', cmap='bwr', vmin=9., vmax=15.)
    cax2 = fig.colorbar(cimg2)

    ax3 = fig.add_subplot(7,1,3)
    ax3text = 'Inverse model result - PO4'
    ax3.text(21, 3, ax3text, fontsize=8)
    cimg3 = ax3.imshow(np.mean(inverse_model_PO4, axis=2).T, aspect='equal', interpolation='nearest', origin='lower', cmap='bwr', vmin=9., vmax=15.)
    ...
    cimg4 = ax4.imshow(np.mean(inverse_model_blood, axis=2).T, aspect='equal', interpolation='nearest', origin='lower', cmap='bwr', vmin=9., vmax=15.) # Residuals
    cax4 = fig.colorbar(cimg4)

    ax5 = fig.add_subplot(7,1,5)
    ax5text = 'forward model: bloodhist'
    ax5.text(21, 3, ax5text, fontsize=8)
    cimg5 = ax5.imshow(np.mean(forward_model_M1_blood_hist, axis=2).T, aspect='equal', interpolation='nearest', origin='lower', cmap='bwr', vmin=9., vmax=15.)
    ...
    cimg6 = ax6.imshow(np.mean(forward_model_M1_PO4_hist, axis=2).T, aspect='equal', interpolation='nearest', origin='lower', cmap='bwr', vmin=9., vmax=15.) # Residuals
    ...
    cimg7 = ax7.imshow(np.std(inverse_model_PO4, axis=2).T, aspect='equal', interpolation='nearest', origin='lower', cmap='RdGy') # Residuals
