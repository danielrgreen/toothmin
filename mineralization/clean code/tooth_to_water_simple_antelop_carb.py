...
def gen_isomaps(iso_shape, iso_data_x_ct, tooth_model, blood_step, day=-1):
    ...
    for k in xrange(len(model_isomap)):
        model_isomap[k] = model_isomap[k][:,2:,day] + 19.1 #*** No. in middle denotes deletion from bottom PHOSPHATE_OFFSET*** was 18.8
        model_isomap[k] *= 1.02
        model_isomap[k] += 8.3
...
def water_hist_likelihood(w_iso_hist, PO4_t, PO4_pause, PO4_flux, **kwargs):
    ...
    M2_inverse_blood_hist = blood_delta(23.5, w_iso_hist, 35.3, **metabolic_kw)
...
def guess_first(d_tooth, d_O2, d_feed, sample_number):
    ...
    d_tooth_reshape -= 8.3
    d_tooth_reshape /=1.02
    d_tooth_reshape -= 19.1
...
def fit_tooth_data(data_fname, model_fname='equalsize_jul2015a.h5', **kwargs):
    ...
    data_guess = guess_first(data_mean_1D, 23.5, 35.3, 27)
    ...
    M2_inverse_days = np.arange(84., 84.+np.size(M2_inverse_water_hist))
    M2_inverse_blood_hist = blood_delta(23.5, M2_inverse_water_hist, 35.3, **forward_metabolic_kw)
    ...
    print min(M2_inverse_water_hist), max(M2_inverse_water_hist)
    print np.min(M2_inverse_water_hist), np.max(M2_inverse_water_hist)
    ax1.set_xlim(84, 550)
    ...
    for s in list_water_results[:-1]:
        ...
        ax1.plot(days[m2_gestation_curve:], s[:-m2_gestation_curve], 'b-', alpha=0.03)
    ax1.text(350, -18, textstr, fontsize=8)
    ax1.set_ylim(min(M2_inverse_water_hist)-2., max(M2_inverse_water_hist)+2.)
    ax1.set_xlim(84, 550)
    ...
    normal_samples = normal_sampling(data_mean_1D, 23.5, 35.3, 27)
    fig = plt.figure()
    ax1 = fig.add_subplot(1,1,1)
    #ax1.plot(days, water_hist[:days.size], 'k--', linewidth=1.0) # **************** sin curve here **********
    ax1.plot(days[:m2_gestation_curve], first_result_water, 'b-.', linewidth=2.0)
    ...
    for s in list_water_results[:-1]:
        ...
        ax1.plot(days[m2_gestation_curve:], s[:-m2_gestation_curve], 'b-', alpha=0.03)
    ax1.text(350, -6, textstr, fontsize=8)
    ax1.set_ylim(np.min(M2_inverse_water_hist)-6., np.max(M2_inverse_water_hist)+6.)
    ax1.set_xlim(84, 550)
    ax2 = ax1.twiny()
    ax2.plot(np.linspace(0, 42, len(normal_samples)), normal_samples, 'ko')
    ax2.set_xlim(0, 42)
    ax2.set_ylim(min(M2_inverse_water_hist)-6., max(M2_inverse_water_hist)+3.)
    ...
    ax2.plot(np.linspace(0, 42, len(normal_samples)), normal_samples, 'ko')
    ax2.set_xlim(0, 42)
    ax2.set_ylim(min(M2_inverse_water_hist)-3., max(M2_inverse_water_hist)+2.)
...
def main():
    ...
    fit_tooth_data('/Users/darouet/Documents/code/mineralization/clean code/s40_data.csv')
