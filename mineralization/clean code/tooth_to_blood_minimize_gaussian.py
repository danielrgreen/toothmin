...
def fit_tooth_data(data_fname, model_fname='final_equalsize_dec2014.h5', **kwargs):
    ...
    local_opt.set_lower_bounds([-9., -23., 20, 40])
    local_opt.set_upper_bounds([-5., -15., 35, 70])
    local_opt.set_min_objective(f_objective)

    global_opt = nlopt.opt(nlopt.G_MLSL_LDS, 4)
    global_opt.set_maxeval(10000)
    global_opt.set_lower_bounds([-9., -23., 20, 40])
    global_opt.set_upper_bounds([-5., -15., 35, 70])
    ...
    x_opt = global_opt.optimize([-8., -16., 25., 60.])
    ...
    real_switch_hist = water_4_param(-6.45, -19.35, 51, 95)
    ...
    ax.plot(days, real_switch_hist, 'k-.', linewidth=1.0)
    ax.plot(days, w_iso_hist, 'b', linewidth=3.0)
    ax.plot(days, blood_hist, 'r', linewidth=3.0)
    ...
    fig.savefig('bloodhist-{0}.svg'.format(t_save), dpi=150, bbox_inches='tight')
    plt.show()
