...
def compare(model_isomap, model_isomap_2, model_isomap_3, data_isomap, w_iso_hist, prior_rate, score_max=1000., data_sigma=0.20, sigma_floor=0.05):
    ...
    score = (mu - data_isomap)**2 / (2*sigma**2) +np.log(sigma)
    #score[score==0.] = 1.
    ...
    prior_len = w_iso_hist.size
    score_len = np.sum(~np.isnan(data_isomap))
    prior_weight = score_len / float(score_len+prior_len)
    score_weight = prior_len / float(score_len+prior_len)

    print 'prior weight', prior_weight
    print 'score weight', score_weight

    print 'p_final = ', (score*score_weight)+(prior_score_rate*prior_weight)
    #return 0
    return (score*score_weight)+(prior_score_rate*prior_weight)
...
def prior_rate_change(w_iso_hist, rate):
    ...
    diff_water = np.diff(w_iso_hist)**2
    prior_score = diff_water/(2*rate**2)
    #prior_score[prior_score==0.] = 1.
    prior_score[np.isnan(prior_score)] = 0.
    prior_score = np.sum(prior_score)
